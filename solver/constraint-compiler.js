...
// A requirement may only be marked as supported when the solver has a final
// validator for it. Keep known-but-not-yet-implemented EA types out of this
// set so new challenges fail closed instead of being submitted unchecked.
const IMPLEMENTED_REQUIREMENT_TYPES = new Set(
  REQUIREMENT_KEYS.filter(
    (type) => type !== "legend_count" && type !== "num_trophy_required",
  ),
);
...
export const normalizeRequirementType = (rule) => {
  ...
  const type =
    ...
    null;
  if (type && IMPLEMENTED_REQUIREMENT_TYPES.has(type)) return type;
