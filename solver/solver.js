...
import { applyPlayerValuePolicy } from "./player-policy.js";
...
export const buildSolverContext = ({
  ...
  normalizedPlayers = applyPlayerValuePolicy(
    normalizedPlayers,
    normalizedPrioritize,
  );
  return {
...
const runPipeline = (inputContext, seed = null, phaseConfig = null) => {
  ...
  // Keep the requested size authoritative. Capping it to the pool size made
  // an 11-player challenge with only 10 candidates appear solved.
  ...
  const buildFailingRequirements = (workingSquad, currentChemistry) => {
    ...
    if ((workingSquad?.length ?? 0) < squadSize) {
      failing.push({
        ...
        op: "exact",
        count: squadSize,
        current: workingSquad?.length ?? 0,
        reason: "insufficient_player_pool",
      });
    }
    for (const unsupported of compiledConstraints.unsupportedRules || []) {
      failing.push({
